...
[网页版](https://bhtx.prom1se.cn) · [QQ 群机器人](#-qq-群机器人) · [数据看板](https://bhtx.prom1se.cn/dashboard) · [架构详解](PROJECT.md)

</div>

---

小程序时代 1900+ 用户、100+ 次真实拼成。小程序停止服务后，整个系统以 **网页 + QQ 群机器人** 的形态重建：发布行程从填表变成说一句话。
...
在群里 @机器人 即可完成一切，规则引擎优先、免费 LLM 兜底理解口语：
...
| 我的 | 查看进行中的行程 |
| 完成 行程号 / 取消行程 行程号 | 发起人结束行程，成员收通知 |
| 车费 行程号 金额 | 任意成员填写实际车费 |
| 通知 行程号 / 播报 | 提醒同车成员 / 播报我的行程到全部群 |
| 私聊 绑定 学号 · 联系方式 · 解除绑定 | 身份与联系信息管理 |

完整功能、指令与上下文：见 [PROJECT.md](PROJECT.md)。
...
- **行程号**：`YYMMDD + 当天第几班`（如 `260914001`），一经分配不变——群里凭号即可精确操作，不依赖上下文
- **同路推荐**：发布瞬间匹配相近到达地点（如乐多港万达 ↔ 昌平西山口）+ 1 小时内的其他行程，把“重复发车”变成“拼上已有车”
- **统一口径**：容量按乘客数计（司机不计人），全站人数统一 `x/y 人`（x 含发起者）
- **单一实现**：网页与机器人共用同一套后端规则——机器人经内部接口以身份凭证自调用公开 API，限流、防超卖、脱敏天然一致

## 🛠 技术栈与架构

零构建前端（Vue 3 单页）+ 单文件后端（Node 20 / Express / Mongoose）+ QQ 官方机器人（WebSocket 长连接 + 规则解析 + 免费 LLM 兜底）。
...
架构、数据模型、安全边界的完整说明见 [PROJECT.md](PROJECT.md)。
...
发布：`python tools/deploy.py <密码> upload && python tools/deploy.py <密码> start`
...
- 后台与管理接口全部 ADMIN_KEY 鉴权；内部接口不对公网暴露逻辑
- LLM 仅做意图理解、输出受限 JSON 经代码三重校验，不直接生成用户内容、不接触个人敏感信息
- 行为埋点匿名统计、90 天自动过期；平台不经手资金，详见站内[隐私政策](https://bhtx.prom1se.cn/#/legal)
