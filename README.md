...
- **说句话就能发布**：@机器人 说人话即可创建行程——中文时间（数字、时段、星期）、地点关键词、口语查询都能解析
- **LLM 只当耳朵，不当嘴**：仅当规则解析完全失效时才调用免费 LLM，且它只输出受限 JSON 意图，经代码三重校验（意图白名单 / 地点库 / 时间合法）后复用既有流程；用户可见内容永远由代码生成，杜绝幻觉
- **行程号直达操作**：`YYMMDD + 当天第几班`（如 `260914001`），一经分配不变；凭号即可加入 / 退出 / 填车费，不依赖聊天上下文
- **同路推荐**：发布瞬间匹配「到达地点相近 + 1 小时内出发」的其他进行中行程，把「重复发车」变成「拼上已有车」
- **统一容量口径**：按乘客数计（司机不计人），全站人数统一 `x/y 人`（x 含发起者）
- **单一规则实现**：网页与机器人共用同一个后端——机器人以用户身份凭证自调用公开 API，限流、防超卖、脱敏天然一致，永不漂移
